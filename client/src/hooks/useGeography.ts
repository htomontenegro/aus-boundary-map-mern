...
export function useGeography() {
  ...
  useEffect(() => {
    fetch('/geographies.json')
      ...
      .then((cfg: GeographyConfig) => {
        setConfig(cfg);
        setSelection(s => ({ ...s, scope: cfg.defaultScopeId }));
      })
      .catch(err => console.error('Failed to load geography config:', err));
  }, []);

  const activeScope = config?.scopes.find(s => s.id === selection.scope) ?? null;
  ...
  return { config, selection, setSelection, activeScope, activeArea, resolvedNode, boundaryUrl };
}
