...
export default function SidebarPanel({ entries, categoriesById, selectedEntryId, onEntryClick, onBack }: Props) {
  return (
    <aside className="w-72 flex-shrink-0 bg-white border-r border-gray-200 flex flex-col overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
        {selectedEntryId && onBack ? (
          <button onClick={onBack} className="text-xs text-blue-600 hover:text-blue-800 flex items-center gap-1">
            ← Back
          </button>
        ) : (
              ...
              return (
                <li
                  ...
                  className={`flex gap-3 p-3 cursor-pointer transition-colors ${
                    entry.id === selectedEntryId ? 'bg-blue-50' : 'hover:bg-gray-50'
                  }`}
                >
                  {entry.image ? (
                    <img
